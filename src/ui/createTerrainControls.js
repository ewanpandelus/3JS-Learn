 ...
 * Internal: renders a minimal control set, batches rapid slider updates per frame, and emits typed setting patches.
 */
export function createTerrainControls(initialSettings, onChange) {
  ...
  panel.innerHTML = `
    ...
    <p style="margin:0 0 10px;opacity:.82;line-height:1.35;">Adjust core noise values and terrain colors.</p>
   ...
   * Internal: maps color/integer/float controls to the expected runtime setting types.
   */
  function parsePatch(target) {
    ...
    if (valueBubble && target.type !== 'color') {
   ...
   * Inputs: native DOM `change` event from color/number controls.
